...
class MS_Median():
    ...
    def select_labeled_point(self):
        '''
        select labeled instance based on Expected Class Interval Minimization
        '''
        for idx in self.intlabeled:
            if len(self.Xin[idx].inter) <= 3:
                ...
                for lab in self.Xin[idx].inter[1:-1]:
                    tmp_eis = 0.0
                    for ele, prob in self.Xin[idx].prob.items(): # 假设的真实标记为ele
                        if ele < lab:
                            tmp_eis += prob * (lab - self.Xin[idx].inter[0])
                        elif ele > lab:
                            tmp_eis += prob * (self.Xin[idx].inter[-1] - lab)
                        elif ele == lab:
                            tmp_eis += prob
                    EIS[lab] = tmp_eis
                self.Xin[idx].evalab = min(EIS,key=EIS.get)
...
if __name__ == '__main__':
    ...
    for name in names_list:
        ...
        save_path = Path(r"E:\EEEEE\ALResults\MS-EIM")
