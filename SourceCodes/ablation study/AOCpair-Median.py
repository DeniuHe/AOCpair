...
class MS_Median():
    ...
    def calculate_unlabeled_intlabeled_proba(self):
        ...
        if self.intlabeled:
            for idx in self.intlabeled:
                print("区间样本{}的概率为{}".format(idx,self.model.predict_proba(self.X[[idx]])))
                print("区间样本{}的距离为{}".format(idx,self.model.distant_to_theta(self.X[[idx]])))
    ...
    def select_labeled_point(self):
        '''
        select the median labeled point
        :return:
        '''
        ids_list = self.intlabeled
        for idx in ids_list:
            self.Xin[idx].evalab = self.Xin[idx].inter[int(np.ceil(len(self.Xin[idx].inter) / 2))-1]
            print("区间标记样本{}".format(idx), "  区间：{}".format(self.Xin[idx].inter),
                  "匹配类别：{}".format(self.Xin[idx].evalab), " 真实标记：{}".format(self.y[idx]))
...
if __name__ == '__main__':
    ...
    for name in names_list:
        ...
        save_path = Path(r"E:\EEEEE\ALResults\MS-Median")
