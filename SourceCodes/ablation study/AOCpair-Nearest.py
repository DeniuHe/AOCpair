...
class MS_Median():
    ...
    def select_labeled_point(self):
        '''
        select labeled instance based on Nearest labeled instance
        '''
        for i, idx in enumerate(self.intlabeled):
            tmp_labeled = []
            for jdx in self.abslabeled:
                if self.y[jdx] in self.Xin[idx].inter:
                    tmp_labeled.append(jdx)
            odx = pairwise_distances_argmin(X=self.X[[idx]],Y=self.X[tmp_labeled])
            tar_idx = tmp_labeled[odx[0]]
            self.Xin[idx].evalab = self.y[tar_idx]
...
if __name__ == '__main__':
    ...
    for name in names_list:
        ...
        save_path = Path(r"E:\EEEEE\ALResults\MS-Nearest")
